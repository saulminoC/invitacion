...
function Divider() {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:12, margin:'32px 0' }}>
      ...
      <svg width="24" height="24" viewBox="0 0 24 24"><path d="M12,2 L13.5,10 L22,12 L13.5,14 L12,22 L10.5,14 L2,12 L10.5,10 Z" fill={G} opacity=".7"/></svg>
    ...
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} style={{ position:'absolute', inset:0, borderRadius:16 }}>
      <defs>
        ...
        <linearGradient id="mg" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#f9f6ef"/><stop offset="100%" stopColor="#f2ede3"/>
        </linearGradient>
      </defs>
    ...
    <svg width={w} height={w} viewBox={`0 0 ${w} ${w}`}>
      <defs>
        <radialGradient id="sg" cx="35%" cy="28%" r="72%">
          <stop offset="0%" stopColor="#fce98a"/><stop offset="30%" stopColor="#e8c030"/>
          <stop offset="65%" stopColor="#b88018"/><stop offset="100%" stopColor="#7a520c"/>
        </radialGradient>
      </defs>
      ...
      <text x="54" y="73" textAnchor="middle" fontSize="44" fontFamily="'Cormorant Garamond','Georgia','Times New Roman',serif" fill="rgba(255,252,225,.92)" fontStyle="italic">M</text>
    </svg>
  );
}
...
function DressCode({ myVote }: { myVote: string | null }) {
  const isNino = myVote === 'nino';
  const isNina = myVote === 'nina';
      ...
      {!hasVoted ? (
        <div>
          <p style={{ textAlign:'center', color:`${DARK}77`, fontSize:14, margin:'0 0 22px', fontFamily:"'Cormorant Garamond','Georgia',serif", fontStyle:'italic' }}>
            Vota primero para descubrir tu color
          </p>
        ...
        <div>
          <div style={{ padding:'28px 20px', borderRadius:16, background:activeBg, border:`1.5px solid ${activeBorder}`, textAlign:'center', marginBottom:16, position:'relative', overflow:'hidden' }}>
            ...
            <div style={{ marginBottom:14, display:'flex', justifyContent:'center' }}>
              <Shirt color={activeColor} active={true}/>
            </div>
...
function InvitationPage() {
  ...
  return (
    <div style={{ background:'linear-gradient(160deg,#f2ede1 0%,#faf7f0 20%,#f8f4ec 100%)', padding:'clamp(32px,6vw,56px) clamp(16px,4vw,24px) 64px', minHeight:'100vh', position:'relative', overflow:'hidden' }}>
      {[{ left:'-10px', top:'10%', size:52, op:.24, flip:false, dur:'4s', del:'0s' }, { left:'-8px', top:'42%', size:40, op:.18, flip:false, dur:'5.2s', del:'.8s' }, { left:'-6px', top:'72%', size:36, op:.15, flip:false, dur:'4.8s', del:'1.4s' }].map((b,i)=>(
      ...
      {[{ right:'-10px', top:'18%', size:48, op:.22, dur:'4.5s', del:'0.5s' }, { right:'-8px', top:'50%', size:38, op:.17, dur:'3.8s', del:'1.8s' }, { right:'-6px', top:'78%', size:34, op:.14, dur:'5.5s', del:'.2s' }].map((b,i)=>(
      ...
      <div style={{ maxWidth:520, margin:'0 auto', position:'relative', zIndex:1 }}>
        ...
        <div style={{ display:'flex', justifyContent:'center', alignItems:'flex-end', gap:6, margin:'36px 0 10px', opacity:.32 }}>
          <Bunny size={26} color={G} opacity={1}/><Bunny size={34} color={G} opacity={1}/><Bunny size={26} color={G} opacity={1} flip/>
        </div>
...
function EnvelopeScene({ phase, onOpen }: { phase:'closed'|'opening'|'opened'; onOpen:()=>void }) {
  ...
  return (
    <div onClick={onOpen} style={{ minHeight:'100dvh', display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:24, background:'radial-gradient(ellipse 80% 70% at 52% 45%,#f4ede0 0%,#e8d4ba 48%,#d8c8aa 100%)', overflow:'hidden', position:'relative', cursor:phase==='closed'?'pointer':'default', userSelect:'none', padding:'20px 16px' }}>
      ...
      <div className="bp" style={{ position:'absolute', left:'2%', bottom:'6%', pointerEvents:'none', animationDuration:'4.5s', animationDelay:'.4s', zIndex:2 }}><Bunny size={40} color={G} opacity={.2}/></div>
      <div className="bp" style={{ position:'absolute', right:'2%', bottom:'6%', pointerEvents:'none', animationDuration:'5s', animationDelay:'1.5s', zIndex:2 }}><Bunny size={36} color={G} opacity={.18} flip/></div>
